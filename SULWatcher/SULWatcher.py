...
class LiberaBot(SASL, SSL, DisconnectOnError, Ghost, Bot):
    ...
    def on_privmsg(self, c, event):
        ...
        command = a.strip()
        if self.channels[self.channel].is_voiced(nick) or self.channels[
            self.channel
        ].is_oper(nick):
            try:
        ...
        if a[0] == self.nickname:
            if len(a) == 2:
                ...
                if self.channels[self.channel].is_voiced(nick) or self.channels[
                    ...
                    self.msg(
                        "Sorry, you need to be voiced to give the bot commands.",
                        nick,
                    )
    ...
    def do_command(self, e, cmd, target):
        ...
        if args[0] == "help":
            ...
            if len(args) >= 4:
                try:
                    ...
                    if re.search(probe, string, re.IGNORECASE):
                        self.msg(
                            '"%s" matches case insensitive regex "%s"'
                            % (string, probe),
                            target,
                        )
                    else:
                ...
                self.msg(
                    ...
                    "regex \bregular ?expression\b'.",
                    target,
                )
                try:
                    # Test MySQL connection
                    sql = (
                        "SELECT l_regex,l_user,l_timestamp FROM "
                ...
                if result:
                    ...
                    try:
                        timestamp = time.strftime(
                            "%H:%M, %d %B %Y",
                            time.strptime(
                                r["l_timestamp"].decode("ASCII"), "%Y%m%d%H%M%S"
                            ),
                        )
                    except ValueError:
                        timestamp = r["l_timestamp"]
                    self.msg(
                        "MySQL connection seems to be fine. Last hit "
                        "was {r[l_user]} matching {r[l_regex]} at {ts}.".format(
                            r=r, ts=timestamp
                        )
                    )
                else:
                    self.msg(
                        "MySQL connection seems to be down. Please restart the bots."
                    )
            ...
            if args[1] in ("regex", "badword"):
                ...
                if len(matches) == 0:
                    self.msg(
                        'There is no regex which matches the string "%s"' % string,
                        target,
                    )
                else:
                    self.msg(
                        '"%s" matches "%s".' % (string, '", "'.join(matches)), target
                    )
            elif args[1] == "adder":
                adder = args[2]
                regexes = []
                sql = "SELECT r_regex FROM regex WHERE r_cloak=%s AND r_active=1;"
                args = (adder,)
                ...
                if len(regexes) == 0:
                    ...
                    maxlen = 20
                    shortlists = [
                        regexes[i : i + maxlen] for i in range(0, len(regexes), maxlen)
                    ]
                    for i in range(0, len(shortlists)):
                        self.msg(
                            r"%s added (%s/%s): %s."
                            % (adder, i + 1, len(shortlists), ", ".join(shortlists[i])),
                            target,
                        )
            ...
            if args[2] in ("regex", "badword"):
                ...
                sql = (
                    "UPDATE regex SET r_regex=%s,r_cloak=%s,"
                ...
                if args[3] == "!":
                    reason = " ".join(args[4:])
                    sql = (
                        "UPDATE regex SET r_reason=%s,r_cloak=%s,"
                        "r_timestamp=%s WHERE r_id=%s"
                    )
                    args = (reason, cloak, time.strftime("%Y%m%d%H%M%S"), index)
                else:
                    reason = " ".join(args[3:])
                    sql = "UPDATE regex SET r_reason=%s,r_timestamp=%s WHERE r_id=%s"
                ...
                if args[3] == "true":
                    sql = "UPDATE regex SET r_case=1,r_timestamp=%s WHERE r_id=%s"
                else:
                    sql = "UPDATE regex SET r_case=0,r_timestamp=%s WHERE r_id=%s"
            ...
            if args[1] in ("badword", "badwords", "regex", "regexes"):
                if self.channels[self.channel].is_oper(nick):
                    ...
                    for i in range(0, len(shortlists)):
                        self.msg(
                            "Regex list (%s/%s): %s"
                            % (i + 1, len(shortlists), ", ".join(shortlists[i])),
                            target,
                        )
                        time.sleep(2)  # sleep a bit to avoid flooding?
                else:
                    self.msg(
                        "Sorry, can't do. I'm afraid of flooding. You "
    ...
    def buildRegex(self):
        ...
        for r in results:
            ...
            try:
                ...
                self.msg(
                    "Disabling regex %s. Could not compile pattern "
    ...
    def addRegex(self, regex, cloak, target):
        ...
        if not result:
            sql = (
                "INSERT IGNORE INTO regex (r_regex,r_cloak,r_timestamp) "
                "VALUES (%s,%s,%s)"
            )
            ...
            if self.sulwatcher.querier.cursor.rowcount > 0:
                self.msg(
                    "%s added %s to the list of regexes. If you would "
                    'like to set a reason, say "SULWatcher: add reason '
                    '%s reason for adding the regex".'
                    % (cloak, regex, self.sulwatcher.querier.cursor.lastrowid),
                    target,
                )
                self.buildRegex()
        else:
    ...
    def removeRegex(self, regex=None, index=None, target=None):
        ...
        if regex:
            ...
            sql = "UPDATE regex SET r_active=0,r_timestamp=%s WHERE r_id=%s;"
            args = (time.strftime("%Y%m%d%H%M%S"), index)
        else:
            return
        self.sulwatcher.querier.do(sql, args)
    ...
    def getPrintRegex(self, regex=None, index=None, target=None):
        ...
        if r:
            ...
            try:
                timestamp = time.strftime(
                    "%H:%M, %d %B %Y",
                    time.strptime(r["r_timestamp"].decode(), "%Y%m%d%H%M%S"),
                )
            except ValueError:
                timestamp = r["r_timestamp"].decode()
            self.msg(
                "Regex %s (#%s, %s, %s hits) added by %s with last "
    ...
    def addToList(self, who, groupname, target):
        ...
        if not group_members:
            ...
            sql = "INSERT INTO setup (s_param,s_value) VALUES (%s,%s);"
            args = (groupname, who)
            self.sulwatcher.querier.do(sql, args)
            if self.sulwatcher.querier.cursor.rowcount > 0:
                ...
                try:
                    ...
                    if not bad and not good:  # Use old method to build bot spam
                        ...
                        for m in matches:
                            try:
                                sql = (
                                    "INSERT INTO logging (l_regex,l_user,"
                                    "l_timestamp) VALUES "
                                    "(%s,%s,%s);"
                                )
                                args = (
    ...
    def get_config_result(self, key):
        ...
        result = [
            ...
            for r in self.querier.do(
                "SELECT s_value FROM setup WHERE s_param = %s;", (key,)
            )
        ]
        if len(result) > 1:
            return result
