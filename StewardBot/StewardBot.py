...
class FreenodeBot(SingleServerIRCBot):
    ...
    def on_welcome(self, c, e):
        ...
        if self.listen and self.listened:
            for chan in self.listened: c.join(chan)
    ...
    def on_privmsg(self, c, e):
        ...
        if a[0]=="@" or a.lower().startswith(self.nickname.lower()+":"):
            if a[0]=="@": command = a[1:]
            else: command = re.sub("(?i)%s:" % self.nickname.lower(), "", a).strip(" ")
            if command.lower()=="die":
                if self.getcloak(e.source())==self.owner: self.do_command(e, command)
                else: self.msg(nocando, nick)
        ...
        if where==self.channel:
            ...
            if a[0]=="@" or a.lower().startswith(self.nickname.lower()+":"):
                ...
                for evilchar in evilchars:
                    if evilchar in a:
                            self.msg("We do not fancy the abusive characters your command contains.", self.channel)
                            return
                #End of Anti-PiR hack
                ...
                if command.lower() in ["die"] or self.startswitharray(command.lower(), ["steward", "huggle", "help", "privileged list", "ignored list", "stalked list", "listen list", "stew users", "stew nicks", "stew optin", "stew info"]):
                    ...
                    pass #if not self.quiet: self.msg("You're not allowed to issue commands.")
    ...
    def do_command(self, e, cmd, target=None):
        ...
        #Help
            ...
            self.msg("Help = http://tools.wmflabs.org/stewardbots/stewardbot.html", nick)
        ...
        #Die
            ...
            if self.getcloak(e.source()) != self.owner:
                if not self.quiet: self.msg("You can't kill me; you're not my owner! :P")
            else:
                self.msg("Goodbye!")
                c.part(self.channel, ":Process terminated.")
                bot2.connection.part(bot2.channel)
                if self.listen and self.listened:
                    for chan in self.listened: self.connection.part(chan, ":Process terminated.")
    ...
    def do_privileged(self, cmd, target, nick):
        if cmd.lower().startswith("list"):
            ...
            if not who:
                ...
                privcloak = query('select p_cloak from privileged where p_nick="%s"' % who)
                if len(privcloak)==0:
                    self.msg("%s is not in the list of privileged users!" % who, target)
                else:
                    self.msg("The cloak of privileged user %s is %s" % (who, privcloak[0]), target)
        elif cmd.lower().startswith("add"):
            who=re.sub("(?i)^add", "", cmd).strip(" ")
            wholist = who.split(" ")
            if len(wholist) < 2:
                if not self.quiet: self.msg("You have to specify a nick and a cloak", target)
            else:
                pnick = wholist[0]
            ...
            wholist = who.split(" ")
            if len(wholist) < 2:
                if not self.quiet: self.msg("You have to specify a nick and a cloak or another nick", target)
            else:
                pnick = wholist[0]
                pcloak = wholist[1]
                renamecloak = False
    ...
    def do_ignored(self, cmd, target, nick):
        if cmd.lower().startswith("list"):
            ...
            wholist = who.split(" ")
            if len(wholist) < 2:
                if not self.quiet: self.msg("You have to specify two usernames", target)
            else:
                iuser1 = wholist[0]
                iuser2 = wholist[1]
    ...
    def do_stalked(self, cmd, target, nick):
        if cmd.lower().startswith("list"):
            ...
            wholist = who.split(" ")
            if len(wholist) < 2:
                if not self.quiet: self.msg("You have to specify two page names", target)
            else:
                ipage1 = wholist[0]
                ipage2 = wholist[1]
    ...
    def do_listen(self, cmd, target, nick):
        if cmd.lower().startswith("list"):
            ...
            if not self.listen and self.listened:
                for chan in self.listened: self.connection.join(chan)
                if not self.quiet: self.msg("Joined the 'listen' channels.", target)
                self.listen = True
        elif cmd.lower().startswith("off"):
            if self.listen and self.listened:
                for chan in self.listened: self.connection.part(chan)
        ...
        if cmd.lower().startswith("users"):
            ...
            if not who:
                ...
                if len(stewinfo)==0:
                    ...
                    if stewinfo[0][0] == None:
                        ...
                        if stewinfo[0][2] == 0: soptin = "n't"
                        else: soptin = ""
                        stewout += ". %s is%s in the list of opt-in nicks." % (stewinfo[0][0], soptin)
            ...
            if not who:
                ...
                if len(query('select s_username from stewards where s_username="%s"' % who))==0:
                    ...
                    self.privileged = query(queries["privcloaks"]) # update the list of privileged cloaks
                    bot2.ignored = query(queries["ignoredusers"]) # update the list of ignored users
                    if not self.quiet: self.msg("%s removed from the list of stewards!" % who, target)
        elif self.startswitharray(cmd.lower(), ["change", "edit", "modify", "rename"]):
            who=re.sub("(?i)^(change|edit|modify|rename)", "", cmd).strip(" ")
            wholist = who.split(" ")
            wllen = len(wholist)
            if wllen < 2:
                if not self.quiet: self.msg("You have to specify two usernames, and optionally nick, cloak and opt-in preference", target)
            else:
                suser1 = wholist[0]
                suser2 = wholist[1]
                ...
                if len(query('select s_username from stewards where s_username="%s"' % suser1))==0:
            	    ...
            	    if len(setlist)==0:
                        ...
                        self.privileged = query(queries["privcloaks"]) # update the list of privileged cloaks
                        bot2.ignored = query(queries["ignoredusers"]) # update the list of ignored users
                        if not self.quiet: self.msg("Updated information for steward %s!" % suser1, target)
        else:
            if not self.quiet: self.msg(self.badsyntax, target)
        ...
        if nick.lower()!="dungodung":           
            ...
            try:
                ...
                if bot1.listen and bot1.listened:
                    for chan in bot1.listened: bot1.connection.part(chan, "Process terminated.")
    ...
    def on_pubmsg(self, c, e):
        ...
        if not bot1.quiet:
            ...
            if "Special:Log/rights" in a:
                ...
                if extra: print "!!! There are extra parameters!"
                comment = found.group('comment')
                if comment: comment = " with the following comment: 07" + comment.strip(" ") + ""
                else: comment=""
                selff=""
                bott=""
                if "@" in usertarget:
                    if usertarget.split("@")[0]==usersource: selff="6(self) "
                elif usersource==usertarget: selff="06(self) "
                if "bot" in state1 or "bot" in state2: bott="06(bot) "
                bot1.msg("%s%s03%s changed user rights for %s from 04%s to 04%s%s" % (selff, bott, usersource, usertarget, state1, state2, comment))
                ...
                if "gblock2" in a:
                    ...
                    comp = re.compile("14\[\[07Special:Log/gblblock14\]\]4 gblock210 02(?P<extra>.*?) 5\* 03(?P<usersource>.+?) 5\* +10(?P<didwhat>.+?) \[\[02User:(?P<usertarget>.+?)10\]\] \((?P<expiry>.+?)\)(: (?P<comment>.+))?", re.DOTALL)
                    found = comp.search(a)
                    print timestamp+who+a
                    if not found: print "!!! Error!"; return
                    ...
                    comp = re.compile("14\[\[07Special:Log/gblblock14\]\]4 modify10 02(?P<extra>.*?) 5\* 03(?P<usersource>.+?) 5\* +10(?P<didwhat>.+?) \[\[02User:(?P<usertarget>.+?)10\]\] \((?P<expiry>.+?)\)(: (?P<comment>.+))?", re.DOTALL)
                    found = comp.search(a)
                    print timestamp+who+a
                    if not found: print "!!! Error!"; return
                    expiry = found.group('expiry')
                    ...
                    comp = re.compile("14\[\[07Special:Log/gblblock14\]\]4 gunblock10 02(?P<extra>.*?) 5\* 03(?P<usersource>.+?) 5\* +10(?P<didwhat>.+?) \[\[02User:(?P<usertarget>.+?)10\]\](: (?P<comment>.+))?", re.DOTALL)
                    found = comp.search(a)
                    print timestamp+who+a
                    if not found: print "!!! Error!"; return
                    expiry=""
                ...
                if extra: print "!!! There are extra parameters!"
                comment = found.group('comment')
                if comment: comment = " with the following comment: 7" + comment.strip(" ") + ""
                else: comment=""
                bot1.msg("03%s %s %s%s%s" % (usersource, didwhat, usertarget, expiry, comment))
                ...
                comp = re.compile("14\[\[07Special:Log/globalauth14\]\]4 (?P<action1>.+?)10 02(?P<extra>.*?) 5\* 03(?P<usersource>.+?) 5\* +10(?P<action2>.+?) global account(.*?)(02)?User:(?P<usertarget>.+?)@global(10)?(.+?)(: (?P<comment>.+))?", re.DOTALL)
                found = comp.search(a)
                print timestamp+who+a
                if not found: print "!!! Error!"; return
                usersource = found.group('usersource')
                ...
                if comment: # Best that I could thought of without changing the structure of this branch
                    if action1=="setstatus":
                        if ":" in comment: ss1 = re.compile("set (?P<s>.+?); unset (?P<u>.+?):", re.DOTALL)
                        else: ss1 = re.compile("set (?P<s>.+?); unset (?P<u>.+)", re.DOTALL)
                ...
                if usersource==usertarget: selff="06(self) "
                else: selff=""
		...
		if True: #else:  
			bot1.msg("%s03%s %s %s%s" % (selff, usersource, action2, usertarget, comment))
                ...
                if action=="groupprms2":
                    outtext=re.sub(r"Special:GlobalUsers/(.+?)\.", r"\1: ", outtext)
                    outtext=re.sub(r"Added (.+?);", r"added 04\1; ", outtext)
                    outtext=re.sub(r"Removed (.+)", r"removed 04\1", outtext)
                    ...
                    outtext=re.sub(r"02Special:GlobalUsers/(.+?)10", r"\1", outtext)
                    outtext=re.sub(r"from (.+?) to (.+)", r"from 04\1 to 04\2", outtext)
                    noco = len(re.findall(":", outtext))
                    if noco >= 1:
                        niz = outtext.split(":")[1:]
                        comment=":".join(niz)
                    ...
                    outtext=re.sub(r"(02)?User:(.+?)(10)? from", r"\2 from", outtext)
                    outtext=re.sub(r"from (.+?) to (.+)", r"from 04\1 to 04\2", outtext)
                    noco = len(re.findall(":", outtext))
                    if noco >= 1:
                        niz = outtext.split(":")[1:]
                    ...
                    outtext=re.sub(r"opt-(.+?) based wiki set (.+?) with", r"04opt-\1 based wiki set \2 with", outtext)
                    ...
                    outtext=re.sub(r"wikis in \"(.+?)\":", r"wikis in \1:", outtext)
                    outtext=re.sub(r"added: (.+?);", r"added: 04\1;", outtext)
                    outtext=re.sub(r"removed: (.+)", r"removed: 04\1", outtext)
                    noco = len(re.findall(":", outtext))
                    if noco >= 4:
                    ...
                    outtext=re.sub(r"set \"(.+?)\" to \"(.+?)\"", r"set \1 to \2", outtext)
                    noco = len(re.findall(":", outtext))
                    if noco >= 1:
                        niz = outtext.split(":")[1:]
                        comment=":".join(niz)
                    ...
                    outtext=re.sub(r"type of \"(.+?)\"", r"type of \1", outtext)
                    outtext=re.sub(r"opt-(.+?) based to opt-(.+?) based", r"04opt-\1 based to 04opt-\2 based", outtext)
                    noco = len(re.findall(":", outtext))
                    if noco >= 1:
                        niz = outtext.split(":")[1:]
                        comment=":".join(niz)
                ...
                comp = re.compile("14\[\[07(?P<page>.+?)14\]\](.+?)diff=(?P<diff>[0-9]+)&oldid=(.+?) 5\* 03(?P<user>.+?) 5\* \((.+?)\) 10(?P<comment>.*)", re.DOTALL)
                found = comp.search(a)
                if not found: print "*** Not the edit type I need ***"; return
                rcpage = found.group('page').strip(" ")
                watched=False
                for pg in self.stalked:
                    if pg in rcpage: watched=True; break
                if not watched: print "*** %s: Not a page I'm watching ***" % rcpage; return
                rcuser = found.group('user').strip(" ")
                if rcuser in self.ignored: print "*** %s: Not a user I need ***" % rcuser; return
                print timestamp+who+a
                rccomment = found.group('comment')
                rcdiff = found.group('diff')
                if not rccomment: comment=section=""
                else:
